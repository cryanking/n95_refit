...
However, non-parametric estimates for interval censored data are not unique; in some regions any baseline hazard matching continuity requirements is equally supported by the data.
...
Because this was an exploratory study, no per-determined p-value threshold was applied.
...
hist(prelim.df  %>% select(Sterilzations) %>% unlist ,  plot=T, main="Times sterilized", xlab=NULL, col= rgb(0.6,1.0,1.0,alpha=0.5))
par(mfrow=c(1,1))
```
